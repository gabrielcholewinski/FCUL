...
instance Arbitrary PPM where
  arbitrary = do
     width <- choose (1,255)
     height <- choose (1,255)
     maxValue <- choose (1,255)
     return $ PPM [width,height,maxValue] $ randomPPM width height maxValue

randomPPM :: Int -> Int -> Int -> [[(Int, Int, Int)]]
randomPPM width height maxValue = alinharListas width $ tuplarListas $ (randomList (width*height*3) maxValue) 

randomList :: Int -> Int -> [Int] 
randomList seed max = map (randomNumber max) [1..seed]

randomNumber :: Int -> Int -> Int
randomNumber maxValue seed = fst $ randomR (0,maxValue) (mkStdGen seed)
...
--prop_twoReversesIsOriginal :: PPM -> Flag -> Property
--prop_twoReversesIsOriginal image flag = (flag == (Flag "-fh" fh) || flag == (Flag "-fv" fv)) ==> image == (aplicarFlags image $ duplicateFlag $ flagToString flag)
